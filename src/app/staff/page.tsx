        ...
        {staff.map((s, i) => (
          <div key={s.name} className={`px-5 py-4 flex items-center gap-4 ${i < staff.length - 1 ? 'border-b border-border' : ''}`}>
            <div className={`w-10 h-10 rounded-full flex items-center justify-center ${s.role === 'owner' ? 'bg-dark' : 'bg-subtle'}`}>
              <span className={`text-xs font-semibold ${s.role === 'owner' ? 'text-green-bright' : 'text-ink'}`}>{s.avatar}</span>
            </div>
            ...
            {s.role === 'owner' && (
              <span className="text-[11px] font-semibold px-2.5 py-1 rounded-full bg-green-pale text-green">
